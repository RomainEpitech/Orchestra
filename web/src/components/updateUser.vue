<template>
    <div class="max-w-4xl mx-auto bg-gray-800/40 backdrop-blur-sm rounded-lg border border-gray-700 shadow-xl overflow-hidden">
        <div class="relative h-36 bg-gradient-to-r from-violet-600 to-indigo-700 flex items-end">
            <div class="absolute inset-0 opacity-10" 
                style="background-size: 40px;"
            />
            
            <div class="p-8 w-full flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 mr-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="p-8">
            <form @submit.prevent="updateProfile" class="space-y-8">
                <div class="bg-gray-700/30 rounded-xl p-6 border border-gray-700 hover:border-gray-600 transition-colors duration-300">
                    <h2 class="text-lg font-semibold text-white mb-6 flex items-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 text-violet-400" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div class="flex flex-col sm:flex-row items-center space-y-6 sm:space-y-0 sm:space-x-8">
                        <div class="relative flex-shrink-0">
                            <div v-if="previewImage" class="w-32 h-32 rounded-full overflow-hidden border-4 border-violet-500/30 shadow-lg">
                                <img :src="previewImage" alt="Avatar utilisateur" class="w-full h-full object-cover" />
                            </div>
                            <div v-else-if="formData.avatar" class="w-32 h-32 rounded-full overflow-hidden border-4 border-violet-500/30 shadow-lg">
                                <img :src="formData.avatar" alt="Avatar utilisateur" class="w-full h-full object-cover" />
                            </div>
                            <div v-else class="w-32 h-32 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center border-4 border-violet-500/30 shadow-lg">
                        ...
                        <div class="flex-1">
                            <label for="avatar-upload" class="block text-lg font-medium text-white mb-3">
                                Choisir une image
                            </label>
                            ...
                            <div class="flex items-center space-x-4">
                                <label class="relative group cursor-pointer inline-flex items-center justify-center bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 py-2 px-4 rounded-lg text-white text-sm font-medium transition-all duration-300 hover:shadow-lg active:scale-95">
                                ...
                                <button 
                                    ...
                                    type="button" 
                                    @click="removeAvatar"
                                    class="group text-sm text-red-400 hover:text-red-300 transition-colors duration-200 flex items-center"
                                >
                        ...
                        <div class="space-y-2 relative group">
                            ...
                            <div class="relative">
                                <input 
                                    ...
                                    required
                                    class="w-full bg-gray-700 border border-gray-600 rounded-lg shadow-sm py-3 px-4 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent transition-all duration-200"
                ...
                <div class="bg-gray-700/30 rounded-xl p-6 border border-gray-700 hover:border-gray-600 transition-colors duration-300">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-gray-800/70 backdrop-blur-sm rounded-lg px-6 py-5 border border-gray-700/50 transition-all duration-300 hover:border-gray-600/50 hover:shadow-md">
                            <p class="text-sm text-gray-400 mb-1">Entreprise</p>
                            <p class="text-lg text-white font-medium">{{ enterpriseName }}</p>
                        </div>
                        <div class="bg-gray-800/70 backdrop-blur-sm rounded-lg px-6 py-5 border border-gray-700/50 transition-all duration-300 hover:border-gray-600/50 hover:shadow-md">
                            <p class="text-sm text-gray-400 mb-1">Rôle</p>
                            <div class="flex items-center">
                                <span 
                                    class="inline-block w-4 h-4 rounded-full mr-2 shadow-sm" 
                                    :style="{ backgroundColor: roleColor }"
                                />
                                <p class="text-lg text-white font-medium">{{ roleName }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row sm:justify-end gap-3 pt-6">
                    <button 
                        type="button"
                        @click="cancelEdit" 
                        class="inline-flex justify-center items-center py-3 px-6 border border-gray-600 shadow-sm text-sm font-medium rounded-lg text-gray-300 bg-transparent hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-gray-500 transition-all duration-200 active:scale-95"
                    >
